...
- QQ群:`811516112`，[🖼二维码](resource/AI动力-GPU云平台群聊二维码.png)，（注意：这几天广告号猖獗，加Q群时**请填写您的学校/单位和专业**，答非所问可能会被视作广告号，造成不便十分抱歉。）
...
## 🎉 New 长期福利活动
- 注：DBC指算力积分，可用于租用算力。新用户奖励总计1000DBC（价格随时间浮动）
1. 新用户补贴：每人1000DBC，海外用户奖励翻倍。
补贴方式：注册并绑定邮箱后，先充值1元，送500DBC，一月内用完500DBC，送剩余500DBC；（以租用机器订单为准）；注册及领取流程请参考[这里](#-注册与新用户奖励)（达到使用量后就可以立即领取，发送订单截图和钱包地址@宣传管理员即可）
2. 推荐用户奖励：每人1000DBC，邀请海外用户奖励翻倍。
奖励方式：先奖励500DBC，被邀请用户一月内用完500DBC，送剩余500DBC；（以租用机器订单为准）；
3. 转发人工智能、深度学习类QQ群、微信群（200人以上），每个群奖励500DBC（同样群间隔2周）；
4. 转发朋友圈或QQ空间获得10个赞，奖励1500DBC每次（间隔2周）；
...
- ![](resource/r12.png)
- ![](resource/r25.png)
13. 填写推荐人地址(谁推荐的就填写Ta的地址，没人推荐的话就跳过)和自己的钱包地址（**文本形式**）并附上余额截图，到[群（点这里加群）](#-AI-Power)里发送消息并`@宣传管理`
...
- 格式如下：
  ...
  [钱包地址和 余额的截图或收到的绑定邮件截图（需要包括收件时间），注意不要泄露私钥]
  ```
  ```
